...
import {  UnixToDate, addRecentSearch, getEndPoint } from './utilities';
...
const ResultPage = () => {
    ...
    useEffect(() => {
        ...
        let endpoint = null
        if (cityName) {
            endpoint = `${getEndPoint()}/api/oneCallWeatherByName?cityName=${cityName}`
        } else {
            endpoint = `${getEndPoint()}/api/oneCallWeatherByLoc?lat=${lat}&lon=${lon}`
        }
